...
const SearchUsersAction = ({ goBack, nodeData }: Props) => {
  const { selectedNodeId, updateNodeConfig } = useWorkflowStore();

  const [actionName, setActionName] = useState(nodeData?.nodeName || "Search Users");
  ...
  const saveAction = () => {
    ...
    const config = {
      nodeType: "search_users",
      nodeName: actionName,
      nodeIcon: "add_update_fields",
      nodeDescription: `Search user: ${email || phoneNumber}`,
      nodeData: {
        email,
        phoneNumber,
      },
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-base font-semibold text-gray-900">Search Users</h2>
          <button
            onClick={goBack}
        ...
        <p className="text-sm text-gray-500">Searches for users by email or phone number.</p>
      </div>
