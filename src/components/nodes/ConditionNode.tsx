...
export const ConditionNode: React.FC<NodeProps> = (props) => {
  ...
  const conditionCount = config?.properties?.length || 0;
  ...
  const filterCount = segments.reduce((sum: number, seg: any) => {
    if (!seg || !Array.isArray(seg.filters)) return sum;
    return sum + seg.filters.filter((f: any) => f && f.type && f.type !== "").length;
  }, 0);
    ...
    >
          ...
          <>
            <div className="flex items-center gap-2 flex-1 min-w-0">
              {isRouterPath ? (
                // Router path display
                <>
                  <div className="px-2 py-1 rounded-md bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center shadow-sm flex-shrink-0">
                ...
                // If/Else branch display
                <>
                  <div className="px-2 py-1 rounded-md bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center shadow-sm flex-shrink-0">
                    <span className="text-[11px] font-semibold text-white tracking-wide">
                      {branchType === "if" ? "If" : branchType === "elseif" ? "Else If" : "Else"}
                    </span>
                  </div>
                  <div className="flex flex-1 min-w-0">
                    {branchType !== "else" && filterCount > 0 && (
      ...
      {config && (
        <button
          ...
          className="absolute -bottom-6 left-1/2 -translate-x-1/2 w-10 h-10 rounded-full bg-gradient-to-br from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 flex items-center justify-center shadow-lg hover:shadow-xl transition-all hover:scale-110 z-10 border-4 border-white"
        >
